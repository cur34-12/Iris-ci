			...
			$logo_path = 'assets/img/profile';
				...
				if ($this->form_validation->run() === FALSE) {
					...
					if($this->ion_auth->update($user->id, $data)) {
						...
						redirect('profile/user', 'location', 301);   
		...
		if (!empty($key) && !empty($new_email)) {
			...
			if ($user_email) { 
					...
					if ($change) {
						...
						$data['message']=(validation_errors() ? validation_errors() : ($this->ion_auth->errors() ? $this->ion_auth->errors() : $this->session->flashdata('message')));
						$sections = array('body_content' => $this->setting['current_theme'] . '/backend/login/reset_email', 'nav_content' => $this->setting['current_theme'] . '/backend/login/nav', 'header_content' => $this->setting['current_theme'] . '/backend/login/header', 'footer_content' => $this->setting['current_theme'] . '/backend/login/footer');
						$this->template->load($this->setting['current_theme'] . '/backend/masterpage', $sections, $data); 
		...
		if ($data['userinfo']){
			...
			$data['min_password_length'] = $this->config->item('min_password_length', 'ion_auth'); 
				...
				if ($this->form_validation->run() == FALSE)	{
					...
					$data['message']=(validation_errors() ? validation_errors() : ($this->ion_auth->errors() ? $this->ion_auth->errors() : $this->session->flashdata('message')));
					$sections = array('body_content' => $this->setting['current_theme'] . '/backend/login/reset_password', 'nav_content' => $this->setting['current_theme'] . '/backend/login/nav', 'header_content' => $this->setting['current_theme'] . '/backend/login/header', 'footer_content' => $this->setting['current_theme'] . '/backend/login/footer');
					$this->template->load($this->setting['current_theme'] . '/backend/masterpage', $sections, $data);	
					...
					if ($change){ 
						...
						$this->notify->notify_forgotten_password_complete('reset_password', $data['userinfo']->email, $data['userinfo']->username, $this->input->post('new_password')); 
						$this->ion_auth->clear_forgotten_password_code($code);
						$sections = array('body_content' => $this->setting['current_theme'] . '/backend/login/reset_password_complete', 'nav_content' => $this->setting['current_theme'] . '/backend/login/nav', 'header_content' => $this->setting['current_theme'] . '/backend/login/header', 'footer_content' => $this->setting['current_theme'] . '/backend/login/footer');
						$this->template->load($this->setting['current_theme'] . '/backend/masterpage', $sections, $data);			 
	...
	function calendar_settings() {
	    ...
	    if ($this->ion_auth->logged_in() || $this->ion_auth->is_admin()) {
			...
			if ($this->input->post('calendar_cancel')) { 
				...
				}else{redirect('profile/user', 301);}			
			} 
			...
			if ($this->input->post('calendar_submit')) { 
				...
				if ($this->form_validation->run() === FALSE) {
					...
					$this->Member_model->fullCalendarSettings($data['userinfo']->id, $language, $timezone, $defaultview, $header_left, $header_center, $header_right, $aspectratio,$hiddendays, $firstday, $businessdays, $businessstart, $businessend, $weeknumbers, $eventlimit, $alldayslot, $slotlabeling, $slotlabelformat, $slotduration, $isrtl, $mintime,$maxtime);

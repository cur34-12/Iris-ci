...
class Home extends CI_Controller {
    ...
    */
    public function rjson () {  	 
		...
		if ($this->ion_auth->logged_in() || $this->ion_auth->is_admin()) {  
			$user = $this->ion_auth->user()->row(); 
			$this->calendar->jsonResources($user); 
		}else {			
			debug('Initialize index - loading "login/index" view');
			redirect('/profile/login', 'location', 301);
		}			
    ...
    */
	public function jsongroups()	{ 	  
		...
		if ($this->ion_auth->logged_in() || $this->ion_auth->is_admin()) { 
            ...
            echo $this->calendar->jsonEventsGroups($group, $user);   
		}else {			
			debug('Initialize index - loading "login/index" view');
			redirect('/profile/login', 301);
		}
				...
				if($endrecurring && $rec==1 || $rec==7 || $rec==14 || $rec==30 || $rec==365){ 
				   $allDay = $allDay;
					...
					foreach ($recur as $dates) {   
						...
						$this->calendar->add_event($recur_id, $title, $marker_category, $backgroundColor, $borderColor, $textColor, $description, $start_datetime, $end_datetime, $url, $allDay, $rendering, $overlap, $recurring, $endrecurring, $auth, $location, $markers_lat, $markers_lng, $username);

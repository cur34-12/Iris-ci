	...
	function index() { 
		...
		if ($this->ion_auth->logged_in() || $this->ion_auth->is_admin()) {	
			...
			if ($data['categories']) { 
				...
				debug('Initialize index - loading "profile/categories/index" view');
				...
				debug('Initialize page empty - loading "profile/categories/empty" view');
	...
	function add() {
		...
		if ($this->ion_auth->logged_in() || $this->ion_auth->is_admin()) {	
			...
			if ($this->input->post('submitCancel')) { 
				redirect('profile/categories', 'location', 301);	
			} 		
			...
			if ($this->input->post('submitAdd')) {
				...
				debug('Initialize index - loading "profile/categories/add" view');
				$sections = array('body_content' => $this->setting['current_theme'] . '/backend/categories/add', 'nav_content' => $this->setting['current_theme'] . '/backend/categories/nav', 'header_content' => $this->setting['current_theme'] . '/backend/categories/header', 'footer_content' => $this->setting['current_theme'] . '/backend/categories/footer');
				$this->template->load($this->setting['current_theme'] . '/backend/masterpage', $sections, $data);
			}			
    ...
    */	
    function edit($id) { 
		...
		if ($this->ion_auth->logged_in() || $this->ion_auth->is_admin()) {	
			...
			if(empty($data['editcategory'])) { redirect('profile/categories', 'location', 301); } 
			...
			if ($this->input->post('submitEdit')) {			
				...
				if ($this->form_validation->run() === FALSE) {
					...
					$this->Category_model->update($id, $user->username, $category_name, $category_desc, $category_bgcolor, $category_bcolor, $category_color );
					$data['message'] = lang('categories_message_success');
    ...
    */		
	function del($id) { 
		...
		if ($this->ion_auth->logged_in() || $this->ion_auth->is_admin()){ 
			...
			if ($data['category']) {  
				$this->Category_model->delete($data['category']->category_id);	 
				// reload the form
				redirect('profile/categories', 'location', 301);   
			}
...
/* Location: ./application/controllers/profile/categories.php */
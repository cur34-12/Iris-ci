...
class Settings extends CI_Controller {
    ...
    function index() {
		...
		$data['cal_schedulerkey'] = $this->setting['cal_schedulerkey'];		
	    ...
	    if ($this->ion_auth->is_admin()) { 
			...
			if ($this->input->post('settings_cancel')) { 
				...
				}else{redirect('admin/settings', 'location', 301); }			
			}   
			...
			if ($this->input->post('settings_submit')) { 
					...
					array(
						'field' => 'cal_apikey',
						'label' => lang('settings_form_googlekey'),
						'rules' => 'trim|xss_clean'
					),
					array(
						'field' => 'cal_schedulerkey',
						'label' => lang('settings_form_schedulerkey'),
						'rules' => 'trim|xss_clean'
					) 
					);
				...
				if ($this->form_validation->run() === FALSE) {
					...
					$this->Setting_model->updateSetting('cal_schedulerkey', $this->input->post('cal_schedulerkey'));
	...
	function calendar_settings() {
	    ...
	    if ($this->ion_auth->is_admin()) { 
			...
			$data['slotlabelformat'] = $this->setting['cal_slotlabelformat'];  			 
			...
			if ($this->input->post('calendar_submit')) { 
				...
				if ($this->form_validation->run() === FALSE) {
					...
					// form not submitted so just show the form 
					debug('Initialize index - loading "settings/calendar_settings" failed view');
					$data['message']=(validation_errors() ? validation_errors() : ($this->ion_auth->errors() ? $this->ion_auth->errors() : $this->session->flashdata('message')));
					...
					// reload the form 
					debug('Initialize index - loading "settings/calendar_settings" validation successful view');
					redirect('admin/settings/calendar_settings', 'location', 301); 
	...
	function theme() {
	    ...
	    if ($this->ion_auth->is_admin()) { 
			...
			if ($this->input->post('theme_submit')) {
				...
				array(
					'field' => 'theme',
					'label' => lang('theme_name'),
					'rules' => 'trim|required|xss_clean'
				),
				array(
					'field' => 'language',
					'label' => lang('theme_language'),
				...
				if ($this->form_validation->run() === FALSE) { 
					...
					redirect('admin/settings/theme', 'location', 301);  
	    ...
	    if ($this->ion_auth->is_admin()) { 
			...
			if ($this->input->post('template_submit')) { 
				...
				if ($this->form_validation->run() === FALSE) { 
					...
					redirect('admin/settings/template/'.$notify_type, 'location', 301);  

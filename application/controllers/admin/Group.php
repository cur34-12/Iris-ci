	    ...
	    if ($this->ion_auth->is_admin()) {  
			...
			$data['users_count'] = $this->Member_model->countUsers();
				...
				if ($this->form_validation->run() === FALSE) { 
					...
					$sections = array('body_content' => $this->setting['current_theme'] . '/backend/group/add', 'nav_content' => $this->setting['current_theme'] . '/backend/group/nav', 'header_content' => $this->setting['current_theme'] . '/backend/group/header', 'footer_content' => $this->setting['current_theme'] . '/backend/group/footer');
					$this->template->load($this->setting['current_theme'] . '/backend/masterpage', $sections, $data);   
    ...
    */	
    function edit($id) {  
	    ...
	    if ($this->ion_auth->is_admin()) {  
			...
			$data['readonly'] = ($this->config->item('admin_group', 'ion_auth') === $data['group']->name || $this->config->item('default_group', 'ion_auth') === $data['group']->name) ? 'readonly' : ''; 	  
    ...
    */		
	function del_selected() {  
	    ...
	    if ($this->ion_auth->is_admin()) { 
			...
			$check_defaults = ($this->config->item('admin_group', 'ion_auth') === $groups->name || $this->config->item('default_group', 'ion_auth') === $data['group']->name) ? TRUE : FALSE;

...
class Categories extends CI_Controller {
    ...
    */
	function index() { 
		...
		if ($this->ion_auth->is_admin()) { 
			...
			$data['categories'] = $this->Category_admin_model->get_categories(0, $this->uri->segment(4));	
			...
			if ($data['categories']) { 
				// show the custom categorieslist page
				debug('Initialize index - loading "admin/categories/index" view');
				$sections = array('body_content' => $this->setting['current_theme'] . '/backend/categorieslist/index', 'nav_content' => $this->setting['current_theme'] . '/backend/categorieslist/nav', 'header_content' => $this->setting['current_theme'] . '/backend/categorieslist/header', 'footer_content' => $this->setting['current_theme'] . '/backend/categorieslist/footer');
				$this->template->load($this->setting['current_theme'] . '/backend/masterpage', $sections, $data); 
				
			} else {
				// no data... show the 'no categorieslist' page
				debug('Initialize page empty - loading "admin/categorieslist/empty" view');
				$sections = array('body_content' => $this->setting['current_theme'] . '/backend/categorieslist/empty', 'nav_content' => $this->setting['current_theme'] . '/backend/categorieslist/nav', 'header_content' => $this->setting['current_theme'] . '/backend/categorieslist/header', 'footer_content' => $this->setting['current_theme'] . '/backend/categorieslist/footer');
				$this->template->load($this->setting['current_theme'] . '/backend/masterpage', $sections, $data);
			}				
    ...
    */
 	public function get_categories() { 
		if ($this->ion_auth->is_admin()) {  
			$categories  = $this->Category_admin_model->get_categories(0, $this->uri->segment(4));	 
	...
	function add() {
		...
		if ($this->ion_auth->is_admin()) { 
			...
			if ($this->input->post('submitCancel')) { 
				redirect('admin/categories', 'location', 301);	
			} 		
			...
			if ($this->input->post('submitAdd')) {
				...
				$config = array(
					...
					array(
						...
						'label' => lang('categories_input_group'),
				...
				if ($this->form_validation->run() === FALSE) { 
					...
					$sections = array('body_content' => $this->setting['current_theme'] . '/backend/categorieslist/add', 'nav_content' => $this->setting['current_theme'] . '/backend/categorieslist/nav', 'header_content' => $this->setting['current_theme'] . '/backend/categorieslist/header', 'footer_content' => $this->setting['current_theme'] . '/backend/categorieslist/footer');
					$this->template->load($this->setting['current_theme'] . '/backend/masterpage', $sections, $data);   
					...
					$this->Category_admin_model->add($category_group, $user->username, $category_name, $category_desc, $category_bgcolor, $category_bcolor, $category_color);
				...
				debug('Initialize index - loading "admin/categorieslist/add" view');
				$sections = array('body_content' => $this->setting['current_theme'] . '/backend/categorieslist/add', 'nav_content' => $this->setting['current_theme'] . '/backend/categorieslist/nav', 'header_content' => $this->setting['current_theme'] . '/backend/categorieslist/header', 'footer_content' => $this->setting['current_theme'] . '/backend/categorieslist/footer');
				$this->template->load($this->setting['current_theme'] . '/backend/masterpage', $sections, $data);
			}			
    ...
    */	
    function edit($id) { 
		...
		if ($this->ion_auth->is_admin()) { 
			...
			$data['editcategory'] = $this->Category_admin_model->getCategoriesById($id); 
			if(empty($data['editcategory'])) { redirect('admin/categories', 'location', 301); } 
			...
			if ($this->input->post('submitEdit')) {			
				...
				if ($this->form_validation->run() === FALSE) {
					...
					debug('validation failed - reload categories with error message(s)');
					$data['message']=(validation_errors() ? validation_errors() : ($this->ion_auth->errors() ? $this->ion_auth->errors() : $this->session->flashdata('message')));
					$sections = array('body_content' => $this->setting['current_theme'] . '/backend/categorieslist/edit', 'nav_content' => $this->setting['current_theme'] . '/backend/categorieslist/nav', 'header_content' => $this->setting['current_theme'] . '/backend/categorieslist/header', 'footer_content' => $this->setting['current_theme'] . '/backend/categorieslist/footer');
					$this->template->load($this->setting['current_theme'] . '/backend/masterpage', $sections, $data);  
					...
					$this->Category_admin_model->update($category_id, $category_group, $category_name, $category_desc, $category_bgcolor, $category_bcolor, $category_color);
					$data['message'] = lang('categories_message_success');
				...
				debug('Initialize index - loading "admin/categorieslist/index" view'); 
				$sections = array('body_content' => $this->setting['current_theme'] . '/backend/categorieslist/edit', 'nav_content' => $this->setting['current_theme'] . '/backend/categorieslist/nav', 'header_content' => $this->setting['current_theme'] . '/backend/categorieslist/header', 'footer_content' => $this->setting['current_theme'] . '/backend/categorieslist/footer');
				$this->template->load($this->setting['current_theme'] . '/backend/masterpage', $sections, $data); 
			}				
	...
	function del_selected() {  
	   ...
	   // check user is logged in with permissions
		if ($this->ion_auth->is_admin()) { 
			...
			for($i=0;$i<=$this->Category_admin_model->countCategories();$i++){
				$del_id = $checkbox[$i]; 
			 	$this->Category_admin_model->delete($del_id);	
			}  
    ...
    */		
	function del($id) { 
		...
		if ($this->ion_auth->is_admin()) { 
			...
			$data['category'] = $this->Category_admin_model->getCategoriesById($id);
			if ($data['category']) {  
				$this->Category_admin_model->delete($data['category']->category_id);	 
				// reload the form
				redirect('admin/categories', 'location', 301);   
			}
